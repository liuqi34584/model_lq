...
def process_file_ply_or_pcd(input_file, npoints=25000, visualize=True):
    if os.path.exists(input_file):
        ...
        return np.array(point_set), np.array(normals_set)
...
if __name__ == '__main__':
    ...
    points, colors = process_file_ply_or_pcd(input_file, npoints=90000, visualize=True)

    # 记录开始时间
    start_time = time.time()
    points, pred_labels = predict(points, colors, num_classes = 5, model_path = r'output\pointnet_2_output\pointnet_2_0526_dataset_AnShanBaiLai.pth')
    end_time = time.time()
    print(f"预测耗时: {end_time - start_time:.4f} 秒")

    # visualize_labels(points, pred_labels, [0,1,2])

    # pred_labels = post_process_labels(points, pred_labels,target_label=0)
    # pred_labels = post_process_labels(points, pred_labels,target_label=1)
    # pred_labels = post_process_labels(points, pred_labels,target_label=2)
    # pred_labels = post_process_labels(points, pred_labels,target_label=1)

    # visualize_point_cloud(points, pred_labels)
